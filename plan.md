...
- 原始数据不进 git；
- 代码必须能从 `data/` 下的 `train/` 和 `test/` 开始复现；若旧版 bundle 仍落在 `data/raw/`，脚本要兼容。
...
- `GR`: 垂直参考 Gamma Ray；
- `Geology`: 地层标签，仅训练集可见；测试集 typewell 只有 `TVT` 和 `GR`。
...
这里可以把它理解成“拿水平井的 GR 去对照垂直参考井的 GR，看看是不是整体往深处或浅处挪了一点”。
...
第一版不要每个点都单独搜 offset，先按一小段一小段来做。这样更稳，也更容易判断是不是噪声。
...
测试集不能直接读到 `Geology`，所以它只适合用来做训练期先验、类别统计、对齐分析和 label-conditioned fallback，不能作为 test 直接特征。

简单说，`Geology` 更像“老师给的参考答案”，可以帮我们理解规律，但不能直接拿来抄到测试集里。
...
如果 Part 3 已经有了基础 GR/typewell 信号，下一步更值得做的是把它们收进一个统一的“baseline 引导修正器”，而不是拆成彼此竞争的独立模型。更具体地说：

- 先分段，再修正：把 hidden interval 分成“短尾段”“长尾段”“中间跳变段”，每种段用不同强度的修正；
- 先打分，再决定修不修：`GR_quality_score` 低就少动，`alignment_confidence` 高才允许强修正；
- 先粗搜，再细搜：offset 先用大步长扫一遍，只对高分窗口再细搜；
- 先回退，再冒进：默认顺序是“保 baseline，再加 GR/typewell 修正”，不要让所有井都吃同一种强修正；
- 远离已知段时降低修正幅度，避免长 hidden interval 被过度外推；
- 把计算预算留给 worst wells 和高误差井，不要平均花在所有井上。

## 9. Phase 5: 模型集成

### 9.1 为什么需要集成

这里的“集成”不是把很多完整模型并排拼接，而是把同一个最终预测器拆成几个有明确职责的修正来源：

- baseline 提供主趋势和安全下限；
- geometry residual 修正轨迹和外推误差；
- GR residual 提供岩性响应；
- typewell residual 提供层位对齐；
- aggressive 版本只是更大的修正幅度，不是另一套完全不同的答案。

### 9.2 候选模型池

| 模型 | 角色 |
|---|---|
| B2 tail slope baseline | control / anchor |
| Geometry HGB residual | first correction layer |
| GR HGB residual | petrophysical correction |
| Typewell alignment residual | geology alignment correction |
| LightGBM residual | stronger tabular correction |
...
```text
pred = baseline + w1 * geometry_residual + w2 * gr_residual + w3 * typewell_residual
```

或者更保守一点：

```text
pred = baseline + gate * correction
```

其中 `gate` 由 `GR_quality_score`、`alignment_confidence`、`baseline_confidence` 决定。

按井路由：

- GR missing high -> baseline/geometry 权重大；
- typewell similarity high -> typewell 修正权重大；
- hidden interval long -> 保守修正；
- baseline confidence low -> 不要让单一强修正接管整条曲线。
...
| 提交类型 | 作用 |
|---|---|
| Conservative | 稳定保底，少修正 |
| Balanced | 主力，baseline + geometry + GR |
| Aggressive | 冲高，typewell + 更强修正 |

当前仓库已经落地第一版可提交闭环，实际使用的是：

- baseline + geometry residual；
- Part 3 route-aware confidence；
- conservative / balanced / aggressive 三个版本；
- postprocess + QA；
- `submission.csv` 生成器。
...
| `scripts/build_part3_diagnostics.py` | GR quality / confidence / routing diagnostics |
...
当前状态：已经实现本地可运行的 ensemble -> postprocess -> final submission 闭环，后续主要优化权重、后处理和 Part 3 路由策略。
