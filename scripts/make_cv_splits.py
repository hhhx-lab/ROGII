...
def main() -> int:
    ...
    lines = [
        ...
        "## Validation Priority",
        "",
        "- Primary leaderboard proxy masks: `original_hidden`, `trailing_short`, `trailing_long`.",
        "- Robustness stress-test masks: `mid_contiguous`, `random_contiguous`, `high_gr_missing`, `high_curvature`.",
        "- The public visible test wells overlap train IDs and must remain sanity checks only; do not tune masks, offsets, or blend weights from those three wells.",
        "",
        "## Leakage Controls",
        "",
        ...
        "- `Geology` is optional on test typewell files; feature builders must work with `TVT/GR` only and add safe missing defaults for Geology-derived columns.",
